...
Feature: Service apply
  ...
  Scenario: Add an elb and an s3
    ...
    And all "elb.create.aws-fake" messages should contain a field "vpc_id" with "fakeaws"
    And all "elb.create.aws-fake" messages should contain a field "name" with "fakeaws-$(name)-elb-1"
    And message "elb.create.aws-fake" number "0" should contain "fakeaws-$(name)-web-1" as json field "instance_names.0"
